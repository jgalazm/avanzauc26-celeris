...
fn sineWave(x: f32, y: f32, t: f32, d: f32, amplitude: f32, period: f32, theta: f32, phase: f32, current_boundary: i32) -> vec3<f32> {
    ...
    eta = rect_suave(y, 30/60, 1.0, 0.0) * eta_b;

    for (var j: i32 = 2; j < 61; j = j + 1) {
        eta = eta + rect_suave(y, f32(30/60 * j), 1.0, 0.0) * eta_b;
    }


    let eta_b = eta;
    var eta_total: f32 = 0.0;

    // On définit le nombre de sources et l'espacement pour couvrir les 30m
    let num_sources: i32 = 60;
    let espacement: f32 = 30.0 / f32(num_sources); // soit 0.5m

    for (var i: i32 = 0; i < num_sources; i = i + 1) {
        // Le premier centre sera à 0.25m, le dernier à 29.75m
        let centre = f32(i) * espacement + (espacement * 0.5);
    
        // Largeur de 1.0m : attention, elles se chevauchent !
        // On réduit un peu le suavizado (0.1) pour garder de la définition
        eta_total = eta_total + rect_suave(y, centre, 1.0, 0.1) * eta_b;
    }

    // Application de la vitesse avec le nombre d'onde physique (wave_k)
    let speed = globals.boundary_g * eta_total / (c * wave_k) * tanh(wave_k * d);
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    if( dry_here == 1 && boundary_boolean < 0) {  // point is wet
        ...
        else if (sum_dry == 1 && (globals.algochanges == 0 || nearshore <= globals.delta)) {  // freeze end of single grid channel, with free surface gradient equal to zero
